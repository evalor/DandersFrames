...
f:SetScript("OnEvent", function(self, event)
    if event == "PLAYER_LOGIN" then
        ...
        -- DELAY INITIALIZATION: Wait a moment to ensure Core has loaded the DB
        C_Timer.After(1, function()
            -- 1. Ensure Database Defaults for the icon exist
            -- Note: DF.db is initialized in DandersFrames.lua (Core).
            -- If Core hasn't run yet, this might be nil.
