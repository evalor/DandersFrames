...
-- NEW: Applies Health Bar Orientation (Left-Right, Right-Left, etc)
function DF:ApplyBarOrientation(frame)
    ...
    -- Sync other status bars (Prediction/Absorb)
    -- We check if they exist and are StatusBars (have SetOrientation)
    local extraBars = {
        frame.myHealPrediction,
        frame.otherHealPrediction,
        frame.totalAbsorb, -- May be a StatusBar in some skins/versions
        frame.myHealAbsorb, -- May be a StatusBar in some skins
        frame.overAbsorbGlow, -- Usually texture, but checked just in case
    }
    
    for _, bar in ipairs(extraBars) do
