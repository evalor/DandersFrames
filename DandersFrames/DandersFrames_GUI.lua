...
function GUI:CreateGradientBar(parent, width, height, db)
    ...
    f.UpdatePreview = function()
        ...
        local function GetC(stage)
            ...
            if not c then return CreateColor(1,1,1,1) end
...
function GUI:CreateColorPicker(parent, label, dbTable, dbKey, hasAlpha, callback)
    ...
    local function UpdateSwatch() if dbTable and dbKey and dbTable[dbKey] then local c = dbTable[dbKey] swatch:SetColorTexture(c.r, c.g, c.b, c.a or 1) end end
    ...
    btn:SetScript("OnClick", function()
        ...
        local info = {
            swatchFunc = function()
                ...
                local a = hasAlpha and ColorPickerFrame:GetColorAlpha() or (dbTable[dbKey].a or 1) 
            ...
            opacityFunc = function()
                dbTable[dbKey].a = ColorPickerFrame:GetColorAlpha()
                UpdateSwatch()
                DF:UpdateAll()
                if callback then callback() end
            end,
            cancelFunc = function(restore)
                dbTable[dbKey].r, dbTable[dbKey].g, dbTable[dbKey].b, dbTable[dbKey].a = restore.r, restore.g, restore.b, restore.opacity
                UpdateSwatch()
                DF:UpdateAll()
                if callback then callback() end
            end,
...
function GUI:CreateDropdown(parent, label, values, dbTable, dbKey, callback)
    local frame = CreateFrame("Frame", nil, parent)
...
function GUI:BuildMainFrame()
    ...
    title:SetText("DandersFrames |cff00aaffv2.0|r")
